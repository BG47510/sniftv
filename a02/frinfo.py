...
# https://github.com/BG47510/  python 3.10

# Config
auth_url = "https://hdfauth.ftven.fr/esi/TA?url=https://simulcast-p.ftven.fr/simulcast/France_Info/hls_monde_frinfo/index.m3u8"
m3u8_path = Path("a02/frinfo.m3u8")
bak_path = Path("a02/bak/frinfo.bak")
TIMEOUT = 10
...
def replace_token_in_text(text: str, old_token: str | None, new_token: str):
    if old_token:
        ...
        # Si aucun ancien jeton n'est découvert, remplacez tout segment correspondant afin d'insérer toujours le nouveau jeton
...
def get_url_from_response(resp: requests.Response) -> str:
    ...
    # Essayez d'abord JSON, mais revenez au texte brut en toute sécurité
    ...
    if "application/json" in ct:
        try:
            ...
            # Clés communes pouvant contenir l'URL
    ...
    # Solution de secours : essayez de trouver une sous-chaîne de type URL dans le corps du texte
    # heuristique simple : recherchez https?://... contenant l'un des marqueurs
...
def main():
    try:
        ...
        if updated == text:
            ...
            # Rien n'a changé — donnez un diagnostic
